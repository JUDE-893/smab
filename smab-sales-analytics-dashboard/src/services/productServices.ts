...
export async function getOrdersProducts(dateRange) {
  const { data } = await axiosClient.post('/proxy/protected', { target: `product/all?timeRange=${dateRange}` });
...
export async function getProductDetails(barcode, year) {
  const { data } = await axiosClient.post('/proxy/protected', { target: `product/analytics?barcode=${barcode}${year ? `&year=${year}` : ''}` });
